...
# 通过部门ID或编码获取部门信息
def get_department_by_id(id: str, type: str) -> Dict:
    r = requests.get(URL + f"/api/openapi/v1/departments/${id}?accessToken={get_access_token()}&departmentBy={type}",
                     headers={"content-type": "application/json", "Accept": "application/json"})
    if r.status_code != 200:
        raise Exception(f"ykb.get_department_by_id: {r.status_code}-{r.text}")
    print(f"ykb.get_department_by_id: {r.text}")
    rsp = r.json()
    return rsp["value"]
...
if __name__ == "__main__":
    ...
    # get_flow_details_by_code("B24001076")
    # print(get_department_by_id("416", "code")["id"])
    # get_flow_details("ID01zMrZVcqgHR")
    ...
    # get_flow_details("ID01AsTcf4qMqP")
    get_flow_details_by_code("B24002098")
    # get_travelmanagement_by_id("ID01yyykA1l3s3")
    # get_specificationId_by_id("ID01vviQDN7OSH")
