...
def handle_multi_dimension(dimensions: list) -> str:
    ...
    for item in dimensions:
        # 处理易快报多选字段
        if (item.find(":") != -1):
            print(ykb.get_staff_by_id(item))
            if "u_OAid" in ykb.get_staff_by_id(item)["staffCustomForm"]:
                l.append(ykb.get_staff_by_id(item)["staffCustomForm"]["u_OAid"])
            else:
                l.append("")
        else:
            l.append(ykb.get_dimension_by_id(item)["code"])  # code字段是类似KH4871的信息
    return ",".join(l)
...
# 处理形如"ID01owxnVpp2h1:ID01qD9zkapWwv"的id
def slice_id(id: str):
    if len(id) == 0:
        return None
    index = id.find(":")
...
workflow_field_map_conf = {
    ...
    "加班申请单": {
        ...
        "mainData": {
            ...
            # 加班类型
            "lb": lambda form: extrawork_map[ykb.get_dimension_by_id(form["details"][0]["feeTypeForm"]["u_加班类型"])["name"]],
            # 附件上传
    ...
    "团建费申请":{
        "workflowId": oa.WORKFLOW_ID_MAP["团队共享激励报销申请流程"],
        "requestName": "title",
        "mainData":{
            # 申请人
            "sqr": lambda form: form["u_申请人ID"],
        ...
        "detailData":{
            "formtable_main_37_dt1":{
                ...
                "field_map":{
                    ...
                    "jssj": lambda item: ykb_date_2_oa_time(item["feeTypeForm"]["u_加班起止时间"]["end"]),
                    # 是否自驾
                    "sfzj":lambda item:item["feeTypeForm"]["u_是否"],
                    # 参与人员
                    "cyry":lambda item:handle_multi_dimension(item["u_团建人员"])
...
if __name__ == "__main__":
    ...
    sync_flow("ID01u8WCxhqlwr", "加班申请单")
